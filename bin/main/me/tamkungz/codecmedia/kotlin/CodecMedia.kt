...
class CodecMediaScope internal constructor(
    ...
    /**
     * Validates [file] using the provided [options].
     * When [options] is `null`, engine defaults are used.
     */
    fun validate(file: String, options: ValidationOptions?) = validatePath(file.toPath(), options ?: ValidationOptions.defaults())
    /** @see validate */
    fun validate(file: File, options: ValidationOptions?)   = validatePath(file.toPath(), options ?: ValidationOptions.defaults())
    /** @see validate */
    fun validate(file: Path, options: ValidationOptions?)   = validatePath(file, options ?: ValidationOptions.defaults())
     ...
     * ```
     */
    fun extractAudio(file: String, outputDir: String) =
        extractAudioPath(file.toPath(), outputDir.toPath(), null)
    /** @see extractAudio */
    fun extractAudio(file: File, outputDir: File) =
        extractAudioPath(file.toPath(), outputDir.toPath(), null)
    /** @see extractAudio */
    fun extractAudio(file: Path, outputDir: Path) =
        extractAudioPath(file, outputDir, null)

    /**
     * Extracts audio using explicit [options].
     * When [options] is `null`, engine defaults are used.
     */
    fun extractAudio(file: String, outputDir: String, options: AudioExtractOptions?) =
        extractAudioPath(file.toPath(), outputDir.toPath(), options)
    /** @see extractAudio */
    fun extractAudio(file: File, outputDir: File, options: AudioExtractOptions?) =
        extractAudioPath(file.toPath(), outputDir.toPath(), options)
    /** @see extractAudio */
    fun extractAudio(file: Path, outputDir: Path, options: AudioExtractOptions?) =
        extractAudioPath(file, outputDir, options)
    ...
    /**
     * Converts [input] to [output] with explicit [options].
     * When [options] is `null`, engine defaults are used.
     */
    fun convert(input: String, output: String, options: ConversionOptions?) =
        convertPath(input.toPath(), output.toPath(), options ?: ConversionOptions.defaults(output.toPath().extensionOrNull()))
    /** @see convert */
    fun convert(input: File, output: File, options: ConversionOptions?) =
        convertPath(input.toPath(), output.toPath(), options ?: ConversionOptions.defaults(output.toPath().extensionOrNull()))
    /** @see convert */
    fun convert(input: Path, output: Path, options: ConversionOptions?) =
        convertPath(input, output, options ?: ConversionOptions.defaults(output.extensionOrNull()))
    ...
    /**
     * Plays [file] with explicit [options].
     * When [options] is `null`, engine defaults are used.
     */
    fun play(file: String, options: PlaybackOptions?) = playPath(file.toPath(), options ?: PlaybackOptions.defaults())
    /** @see play */
    fun play(file: File, options: PlaybackOptions?)   = playPath(file.toPath(), options ?: PlaybackOptions.defaults())
    /** @see play */
    fun play(file: Path, options: PlaybackOptions?)   = playPath(file, options ?: PlaybackOptions.defaults())
...
/**
 * Alias of [probe] for Java API parity.
 */
fun File.get() = codecMedia().get(this)
/** @see File.get */
fun Path.get() = codecMedia().get(this)

/**
 * Reads metadata for this file.
 */
fun File.readMetadata() = codecMedia().readMetadata(this)
/** @see File.readMetadata */
fun Path.readMetadata() = codecMedia().readMetadata(this)

/**
 * Writes metadata for this file.
 */
fun File.writeMetadata(metadata: Metadata) = codecMedia().writeMetadata(this, metadata)
/** @see File.writeMetadata */
fun Path.writeMetadata(metadata: Metadata) = codecMedia().writeMetadata(this, metadata)

/**
 * Writes metadata for this file using the DSL builder.
 */
@CodecMediaDsl
fun File.writeMetadata(block: MetadataBuilder.() -> Unit) = codecMedia().writeMetadata(this, block)
/** @see File.writeMetadata */
@CodecMediaDsl
fun Path.writeMetadata(block: MetadataBuilder.() -> Unit) = codecMedia().writeMetadata(this, block)
...
/**
 * Extracts audio from this file using engine defaults.
 */
fun File.extractAudioTo(outputDir: File) = codecMedia().extractAudio(this, outputDir)
/** @see File.extractAudioTo */
fun Path.extractAudioTo(outputDir: Path) = codecMedia().extractAudio(this, outputDir)

/**
 * Extracts audio from this file using explicit options.
 */
fun File.extractAudioTo(outputDir: File, options: AudioExtractOptions?) =
    codecMedia().extractAudio(this, outputDir, options)
/** @see File.extractAudioTo */
fun Path.extractAudioTo(outputDir: Path, options: AudioExtractOptions?) =
    codecMedia().extractAudio(this, outputDir, options)

/**
 * Extracts audio from this file using DSL options.
 */
@CodecMediaDsl
fun File.extractAudioTo(outputDir: File, block: ExtractBuilder.() -> Unit) =
    codecMedia().extractAudio(this, outputDir, block)
/** @see File.extractAudioTo */
@CodecMediaDsl
fun Path.extractAudioTo(outputDir: Path, block: ExtractBuilder.() -> Unit) =
    codecMedia().extractAudio(this, outputDir, block)
